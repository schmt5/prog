defmodule ProgWeb.UserAuthTest do
  use ProgWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Prog.Accounts
  alias Prog.Accounts.Scope
  alias ProgWeb.UserAuth

  import Prog.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, ProgWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      ProgWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        ProgWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
